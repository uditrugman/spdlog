...
#include "spdlog/sinks/syslog_udp_sink.h"

void replaceDefaultLogger() {
    auto console_sink = std::make_shared<spdlog::sinks::stdout_sink_mt>();
//    auto console_sink = std::make_shared<spdlog::sinks::c>();
    console_sink->set_level(spdlog::level::warn);
    console_sink->set_pattern("[multi_sink_example] [%^%l%$] %v");
    ...
    sockServerAddr.sin_addr.s_addr = inet_addr("10.0.0.5");  /* remote syslogd server ip */
    sockServerAddr.sin_port = htons(514);

    auto syslog_udp_sink = std::make_shared<spdlog::sinks::syslog_udp_sink_mt>(fd,
                                                                               reinterpret_cast<const struct sockaddr *>(&sockServerAddr),
                                                                               (socklen_t) sizeof(sockServerAddr),
                                                                               std::string("my_host"),
                                                                               std::string("my_app"),
                                                                               (1 << 3));
...
//    auto logger = std::make_shared<spdlog::logger>("multi_sink", std::initializer_list<spdlog::sink_ptr>{console_sink, file_sink});
    auto logger = std::make_shared<spdlog::logger>("multi_sink", std::initializer_list<spdlog::sink_ptr>{console_sink,
                                                                                                         syslog_udp_sink});
    logger->set_level(spdlog::level::debug);
    logger->warn("this should appear in both console and file");
    logger->info("this message should not appear in the console, only in the file");
    ...
    spdlog::warn("this should also appear in both console and file");
